...
class RenderingCanvas(app.Canvas):
    ...
    def __init__(self,
        ...
        if interactive:
            ...
            self.program['resolution'] = self._output_size
            self.ensure_timer()
    ...
    def advance_time(self):
        if not self._paused:
            if self._interval == 'auto':
                self.program['time'] = time.clock() - self._clock_time_zero
            else:
                self.program['time'] += self._interval
    ...
    def activate_zoom(self):
        if self._interactive:
            ...
            self.program['resolution'] = (self.physical_size[0], self.physical_size[1])
    ...
    def update_timer_state(self):
        if not self._paused:
            self._clock_time_zero = time.clock() - self.program['time']
            self.ensure_timer()
        else:
            if self._profile:
                self.ensure_timer()
